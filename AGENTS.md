...
- Every `@deepseek-ai/*` runtime package comes from the npm registry, resolved at install time from the dist-tags recorded in [`upstream.json`](upstream.json) — `next` for the dsh family, `latest` for the cordis framework; `pnpm-lock.yaml` is not committed. Never depend on, vendor, or edit upstream source from this repository.
