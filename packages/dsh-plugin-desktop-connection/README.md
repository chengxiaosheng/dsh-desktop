...
- `host-websocket.ts` — the virtual-host WebSocket bridge: patches `window.WebSocket` so constructions targeting the desktop host surface ride the bridge. A plugin that builds host WebSockets from `location.origin` (the harness's own downlink pattern) produces `file:///…` URLs on the desktop's `file://` page — the `file:` → `ws:` protocol swap is a silent no-op in the URL parser, so `new WebSocket('file:///sidebar/ws/…')` would throw the scheme error, and even a correctly-schemed URL would find no server. `resolveWebSocketUrl` maps every `file:`-derived URL and every `ws(s)://dsh.internal` URL to the virtual host and returns a `DesktopWebSocket` shim — the browser WebSocket interface (readyState, on\* handlers, `send`, `close`, addEventListener, binaryType) over the preload bridge — while every other construction keeps the captured native constructor. Messages pushed before the open reply lands are buffered and flushed on open, so a handler's synchronous transcript replay is never lost.
...
The WebSocket half of the bridge contract: `wsOpen({ type: 'ws-open', socketId, url, protocols? })` (invoke, answered `ws-opened`/`ws-failed`), `wsSend`/`wsClose` (one-way), and `onWsEvent` (subscription; events are `ws-message` and `ws-close`). The Electron main runs the registered upgrade route in-process (`webServer.upgrades`, synthesized `GET` request with loopback `Host`/`Origin`) over a bridge-backed `Duplex`, and relays message-level events: server→client frames decoded and pushed, client→server data pushed back as masked frames, and close frames echoed so the closing handshake completes promptly with the real code and reason. The message shapes are mirrored verbatim in the desktop package's `electron/websocket-bridge.ts`.
