 ...
 * `ConnectionController`, restored from the upstream TypeScript source
 * (`sourcePath` at the recorded commit in `upstream.json`).
 *
 ...
 * browser HTTP/WebSocket carrier. The family resolves from the `next` dist-tag
 * at install time; reapply this file from the upstream source whenever
 * `verify:upstream` reports the installed `@deepseek-ai/dsh-client-connection`
 * differs from the recorded version; keep the behavior identical.
 *
 * Two mechanical adaptations from the upstream file, both documented at their
 * site: the type import below (upstream's `./api.ts` re-export shim becomes
 * `@deepseek-ai/dsh-host-apiproxy/api`, the shim's own source), and the
 * constructor's parameter properties spelled as explicit field declarations
 * (parameter properties are not erasable syntax and would fail the Node type
 * stripping the tests rely on).
 */
...
/** host.describe success value (`HostDescription` in the upstream source). */
...
export class ConnectionController {
  ...
  // Adaptation from upstream: `api`/`sinks` are parameter properties there;
  // explicit fields keep the file runnable under Node's type stripping.
