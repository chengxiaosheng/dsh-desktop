/**
 * Pinned copy of the official `@deepseek-ai/dsh-client-connection` client
 * `ConnectionController` (v0.1.0-rc.6), ported to plain JavaScript.
 *
 * The controller is carrier-agnostic: its constructor takes an `IApiClient`
 * (the `events.mux`/`events.host` streams plus `host.describe`), so the desktop
 * IPC carrier drives the same connect/pump/reconnect state machine as the
 * browser HTTP/WebSocket carrier. Reapply this file from the upstream source
 * (`packages/client/connection/src/client/connection.ts`) when the runtime
 * package family bumps; keep the behavior identical.
 */

/**
 * Reconnect/backoff tunables (deployment-varying — no hardcoded tunables;
 * these become the future `ctx.connection` plugin's Config). All fields
...
/**
 * Opens both streams and keeps iterating (pull mode: nothing reads the socket
 * and the tap never fires unless someone for-awaits), reconnecting with
 * exponential backoff on loss. State (generation/attempt) is instance-private,
 * never in the store. The pump body feeds each frame to a sink (sink exceptions
 * must not kill the pump — a broken business layer must not drag down the
 * connection layer).
 */
export class ConnectionController {
  ...
  async loop() {
    while (this.running) {
      ...
      /* v8 ignore next -- initializer placeholder: the Promise executor below
       * runs synchronously and replaces it before anyone can call it. */
      let muxOpened = () => {}
      ...
      try {
        // Strict readiness handshake: describe proves unary reachability,
        // onOpen proves each physical stream is established before any frame —
        // only then may onConnected fire, so the resync it triggers cannot
        // outrun the subscribed baseline. The timeout guards against a carrier
        // that never fires onOpen (see streamOpenTimeoutMs).
