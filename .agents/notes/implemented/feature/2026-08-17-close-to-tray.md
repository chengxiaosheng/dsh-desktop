...
The General settings rows ("关闭窗口行为" / Close-window behavior: quit / minimize to tray, and the host-reboot row) are contributed by the shell's browser half (`src/client/`) through the `settings.general.item` slot, following the current harness row pattern: status-gated rendering (the row hides while the bridge read is unavailable and disables the trigger while loading) and the standard row CSS (including `:hover:not(:disabled)`), shared between the two rows through one `settings-row.module.css` so the restart-host row carries no independent style. Dictionaries live in `src/client/locales.ts` as the `settings.desktop` locale namespace; the same dictionaries carry the tray menu copy. The rows are desktop-only by construction: `dsh-plugin-desktop` mounts only in the desktop composition, so the options never appear in a plain `dsh web` run.

The tray exists on every platform regardless of the setting. A hidden window must always have a restore path, so the tray cannot be conditional on the setting that hides it. The menu (show/restart/quit) is rebuilt at every open, and its labels are published by the renderer over the `dsh:locale` channel at boot and on every locale change — the tray always matches the language the app actually displays (explicit preference or browser-detected), with English standing until the first publication. The tray's "Restart host" item triggers the same in-process host reboot as the settings row (see the [host-reboot note](2026-08-17-host-reboot.md)). Close interception lives in `electron/main.ts`: `before-quit` sets a quitting flag, and the window's `close` handler hides instead of destroying unless a real quit is underway (tray quit, Cmd+Q, or quit while the preference is off). Hidden windows restore through the tray, `second-instance`, and macOS `activate`.
