...
通用设置行（「关闭窗口行为」：退出应用 / 最小化到托盘，以及宿主重启行）由壳层浏览器半区（`src/client/`）经 `settings.general.item` 槽位贡献，遵循当前 harness 行模式：按状态门控渲染（桥读取不可用时隐藏行、加载期间禁用触发器），并使用标准行 CSS（含 `:hover:not(:disabled)`），两行共用同一个 `settings-row.module.css`，因此重启宿主行不携带独立样式。字典位于 `src/client/locales.ts`，注册为 `settings.desktop` locale 命名空间；同一字典也承载托盘菜单文案。这些行天然仅桌面可见：`dsh-plugin-desktop` 只挂载于桌面组合，纯 `dsh web` 运行中永远不会出现。

托盘在任意平台、无论设置如何都始终存在。隐藏的窗口必须有恢复路径，因此托盘不能随「隐藏窗口的设置」而条件性出现。菜单（打开主窗口 / 重启宿主 / 退出）每次打开前重建，其文案由渲染端在启动时与每次语言切换时经 `dsh:locale` 通道发布——托盘始终与应用实际显示的语言一致（显式偏好或浏览器探测皆覆盖），首次发布前以英文兜底。托盘的「重启宿主」项触发与设置行同一条进程内宿主重启路径（见[宿主重启笔记](2026-08-17-host-reboot.zh.md)）。关闭拦截位于 `electron/main.ts`：`before-quit` 置位退出标志，窗口的 `close` 处理器在非真实退出时改为隐藏而非销毁（真实退出包括托盘退出、Cmd+Q、以及偏好关闭时的窗口关闭）。隐藏窗口经托盘、`second-instance` 与 macOS `activate` 恢复。
